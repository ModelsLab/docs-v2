...
Manage subscriptions. All actions require authentication.

<ParamField path="action" type="string" required>
  `list`, `create`, `update`, `cancel`, `pause`, `resume`, `reset-cycle`, `charge-amount`, `offer-cancelation`, `reverse-cancelation`, `fix-payment`
</ParamField>

<ParamField path="subscription_id" type="integer">
  Subscription ID (required for update, cancel, pause, resume, reset-cycle, offer-cancelation, reverse-cancelation, fix-payment)
</ParamField>

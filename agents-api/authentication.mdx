...
| `POST` | `/api/agents/v1/auth/login` | None | Returns Bearer token. Supports `token_expiry` and `scopes` |
| `POST` | `/api/agents/v1/auth/verify` | None | Verify email with `verification_code` (no browser required). Returns `access_token` + `api_key` |
| `POST` | `/api/agents/v1/auth/google-sign-in` | None | Sign in with a Google ID token. Returns Bearer token |
| `GET` | `/api/agents/v1/auth/scopes` | None | Lists every requestable scope and what it grants |
| `POST` | `/api/agents/v1/auth/logout` | Bearer token | Revokes current token. Never scope-gated |
| `POST` | `/api/agents/v1/auth/logout-all` | Bearer token | Revokes all user tokens. Requires `tokens:write` |
| `GET` | `/api/agents/v1/auth/tokens` | Bearer token | Lists tokens with their granted scopes. Requires `tokens:read` |
| `DELETE` | `/api/agents/v1/auth/tokens/{id}` | Bearer token | Revokes one token. Requires `tokens:write` |
| `POST` | `/api/agents/v1/auth/tokens/revoke-others` | Bearer token | Revokes every token except the current one. Requires `tokens:write` |
| `POST` | `/api/agents/v1/auth/switch-account` | Bearer token | Issues a token for another account in your team. Requires `tokens:write` |
...
Account policy may cap what you asked for. Check `token_lifetime_capped` and
...
Omit it and the token is issued with `*`. See [Token Scopes](/agents-api/scopes).

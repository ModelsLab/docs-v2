...
Send a `POST` request to [https://modelslab.com/api/v1/enterprise/flux\_headshot/fetch/\{id}](https://modelslab.com/api/v1/enterprise/flux_headshot/fetch/%7Bid%7D) endpoint to return the corresponding queued images. Where `{id}` is the ID returned together with the image URL in the response upon its generation. This endpoint does not generate new images, it returns already generated/queued images.
